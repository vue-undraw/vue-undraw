...
export default {
  ...
  methods: {
    darken (color) {
      let percent = -20;
      if (color.match(/\b[0-9A-F]{6}\b/gi)) {
        ...
        return "#"+RR+GG+BB;
      } else if (color.match(/\b[0-9A-F]{3}\b/gi)) {
        let R = parseInt(color.substring(1, 2) + color.substring(1, 2), 16);
        let G = parseInt(color.substring(2, 3) + color.substring(1, 2), 16);
        let B = parseInt(color.substring(3, 4) + color.substring(3, 4), 16);
